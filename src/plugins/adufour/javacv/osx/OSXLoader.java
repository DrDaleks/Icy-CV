	...
	public static void initialize()
	{
		if (!initialized)
		{
			...
			if (cl instanceof JarClassLoader)
			{
				...
				jcl.add(OSXLoader.class.getResource("javacv_osx_2.4.2.jar"));
			...
			try
			{
				...
				for (String name : libs)
				{
					...
					// Warning: the following code doesn't work on Mac OS X
					// (libraries are looked for in the Icy root folder, not locally)
					{
					// URL url = OSXLoader.class.getResource(name.replace("2.4", "2.4.2"));
...
//					if (cl instanceof JarClassLoader)
//					{
//						((JarClassLoader) cl).add(OSXLoader.class.getResource(name.replace("2.4", "2.4.2")));
//					}
